...
@Slf4j
public class XSSRequestWrapperByException extends HttpServletRequestWrapper implements XSSRequestWrapperStrategy {

    private final ObjectMapper objectMapper = new ObjectMapper();

    public XSSRequestWrapperByException(HttpServletRequest request) {
        super(request);
    }

    @Override
    public HttpServletRequest wrapRequest(HttpServletRequest request) {
        return new XSSRequestWrapperByException(request);
    }
    ...
    private String sanitizeInput(String string) {
        if (!Jsoup.isValid(string, Safelist.none())) {
